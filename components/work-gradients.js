...
}`;function D(){let t=navigator.userAgent.toLowerCase();return t.includes("safari")&&!t.includes("chrome")&&!t.includes("android")}function W(t){let e=visualViewport?.scale??1,i=visualViewport?.width??window.innerWidth,r=window.innerWidth-t.documentElement.clientWidth,n=e*i+r,o=outerWidth/n,a=Math.round(100*o);return a%5===0?a/100:a===33?1/3:a===67?2/3:a===133?4/3:o}var b={none:0,contain:1,cover:2};var M=`
...
`;function E(t){if(Array.isArray(t))return t.length===4?t:t.length===3?[...t,1]:y;if(typeof t!="string")return y;let e,i,r,n=1;if(t.startsWith("#"))[e,i,r,n]=H(t);else if(t.startsWith("rgb"))[e,i,r,n]=X(t);else if(t.startsWith("hsl"))[e,i,r,n]=k(N(t));else return console.error("Unsupported color format",t),y;return[v(e,0,1),v(i,0,1),v(r,0,1),v(n,0,1)]}function H(t){t=t.replace(/^#/,""),t.length===3&&(t=t.split("").map(o=>o+o).join("")),t.length===6&&(t=t+"ff");let e=parseInt(t.slice(0,2),16)/255,i=parseInt(t.slice(2,4),16)/255,r=parseInt(t.slice(4,6),16)/255,n=parseInt(t.slice(6,8),16)/255;return[e,i,r,n]}function X(t){let e=t.match(/^rgba?\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*(?:,\s*([0-9.]+))?\s*\)$/i);return e?[parseInt(e[1]??"0")/255,parseInt(e[2]??"0")/255,parseInt(e[3]??"0")/255,e[4]===void 0?1:parseFloat(e[4])]:[0,0,0,1]}function N(t){let e=t.match(/^hsla?\s*\(\s*(\d+)\s*,\s*(\d+)%\s*,\s*(\d+)%\s*(?:,\s*([0-9.]+))?\s*\)$/i);return e?[parseInt(e[1]??"0"),parseInt(e[2]??"0"),parseInt(e[3]??"0"),e[4]===void 0?1:parseFloat(e[4])]:[0,0,0,1]}function k(t){let[e,i,r,n]=t,o=e/360,a=i/100,s=r/100,c,h,l;if(i===0)c=h=l=s;else{let u=(m,S,f)=>(f<0&&(f+=1),f>1&&(f-=1),f<.16666666666666666?m+(S-m)*6*f:f<.5?S:f<.6666666666666666?m+(S-m)*(.6666666666666666-f)*6:m),d=s<.5?s*(1+a):s+a-s*a,_=2*s-d;c=u(_,d,o+1/3),h=u(_,d,o),l=u(_,d,o-1/3)}return[c,h,l,n]}var v=(t,e,i)=>Math.min(Math.max(t,e),i),y=[0,0,0,1];var V=!0,Y=.35,j=.45,$=.85,q=.45,K={"google-branding":"Google","ai-mode":"Google","voice-search":"Google","youtube-ai-brand":"YouTube",chirper:"Chirper","peer-payment":"Baton","music-remix":"Baton","baton-branding":"Baton"},G={Google:["#f0edea","#f0edea","#7eb6ff","#7eb6ff","#3d98ff","#1a73e8","#4ec4ff","#c5b8ff","#c5b8ff"],YouTube:["#f0edea","#f0edea","#ff8a8a","#ff8a8a","#ff4d4d","#e60023","#ff7a45","#ffc08a","#ffc08a"],Chirper:["#f0edea","#f0edea","#a8b4ff","#a8b4ff","#6b7cff","#4350e0","#9a7dff","#b8e0ff","#b8e0ff"],Baton:["#f0edea","#f0edea","#7ee0d6","#7ee0d6","#2ec4b6","#0d9488","#5ad4a8","#5ad4a8","#b8ecc0","#b8ecc0"]},Z=G.Google;function J(t){let e=2166136261;for(let i=0;i<t.length;i+=1)e^=t.charCodeAt(i),e=Math.imul(e,16777619);return e>>>0}function g(t,e){return(t>>>e&255)/255}function Q(t,e=""){let i=J(e?`${t}:${e}`:t||"work");return{positions:42+g(i,0)*50,waveX:.22+g(i,8)*.55,waveXShift:g(i,16),waveY:.72+g(i,24)*.28,waveYShift:g(i,4)}}function ee(t){let e=K[t]||"Google";return G[e]||Z}function te(t,e=""){let i=ee(t),r=Q(t,e);return{u_colors:i.map(E),u_colorsCount:i.length,u_positions:r.positions,u_waveX:r.waveX,u_waveXShift:r.waveXShift,u_waveY:r.waveY,u_waveYShift:r.waveYShift,u_mixing:$,u_grainMixer:Y,u_grainOverlay:j,u_fit:b.cover,u_scale:q,u_rotation:0,u_offsetX:0,u_offsetY:0,u_originX:.5,u_originY:.5,u_worldWidth:0,u_worldHeight:0}}var x=new Set;function O(t){let e=new Set([t]),i=t.closest(".project-figure, .project-cover, .project-media");i&&e.add(i),t.parentElement?.classList.contains("media-skeleton")&&e.add(t.parentElement),e.forEach(r=>{r.classList.add("is-loaded"),window.MediaSkeleton?.markLoaded?.(r)})}function B(t){t&&(t._workGradientShader?.dispose?.(),delete t._workGradientShader,delete t.dataset.workGradientInit,t.querySelectorAll?.("canvas")?.forEach(e=>e.remove()),x.delete(t))}function C(t){if(V||!t||t.dataset.workGradientInit==="true")return null;t.dataset.workGradientInit="true";let e=t.dataset.slug||"",i=t.dataset.variant||"";try{let r=new p(t,R,te(e,i),{antialias:!0,alpha:!1,premultipliedAlpha:!0},0,0,2);return t._workGradientShader=r,x.add(t),requestAnimationFrame(()=>{O(t)}),r}catch(r){return delete t.dataset.workGradientInit,console.warn(r),O(t),null}}function U(){V||(document.querySelectorAll("[data-work-gradient]:not([data-work-gradient-init])").forEach(C),queueMicrotask(()=>window.CreamyOrb?.ensureAlive?.()))}function ie(){[...x].forEach(B),x.clear(),document.querySelectorAll("[data-work-gradient]").forEach(t=>{t._workGradientShader?.dispose?.(),delete t._workGradientShader,delete t.dataset.workGradientInit})}function re(t){t&&t.querySelectorAll("[data-work-gradient]").forEach(B)}window.WorkGradients={init:C,initAll:U,dispose:B,disposeIn:re,disposeAll:ie};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",U):U();})();
